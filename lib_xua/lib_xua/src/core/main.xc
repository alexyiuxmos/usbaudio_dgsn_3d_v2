...
extern void dsp_task(chanend c_dsp, chanend c_button, chanend c_led, chanend c_ex3d_started, chanend c_chg_sf, chanend c_flash_rd_req);
...
#include <xassert.h>
extern "C" {
    void dsp_stage0_task(chanend c_dsp);  //stage1 - linked with usb_audio_io/XUA_AudioHub
...
#if (USE_EX3D == 1) || (DSP_TASK == 1)
    chan c_dsp;   //channel link between UBM and dsp_stage1_task
#if (USE_EX3D == 1)
    chan c_button;
    chan c_led;
...
#endif
#if (DSP_TASK == 1)
        xassert(AUDIO_IO_TILE == DSP_TILE);           //DSP pipeline only runs in Tile1
        on tile[DSP_TILE]: dsp_stage0_task(c_dsp);  //stage1 - linked with usb_audio_io/XUA_AudioHub
        // on tile[DSP_TILE]: dsp_stage1_task();
        // on tile[DSP_TILE]: dsp_stage2_task();
        // on tile[DSP_TILE]: dsp_stage3_task();
        // on tile[DSP_TILE]: dsp_stage4_task();
        // on tile[DSP_TILE]: dsp_stage5_task();
#endif
